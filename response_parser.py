...
"""
    ...
    sep_headings = [value.strip().strip(";")
                    for value in headings.split(';\n')]
    ...
    for i, heading in enumerate(sep_headings):
        ...
        next_heading = sep_headings[i + 1] if i + \
            1 < len(sep_headings) else None
        if next_heading:
        ...
        if match:
            structured_summary[heading] = match.group(
                1).strip().strip(':').strip()
        else:
